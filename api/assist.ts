...
import { SYSTEM_PROMPT, buildAssistPrompt } from '../src/ai/prompts';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const body = req.body as { plan: unknown; message: string };
    ...
    const completion = await client.chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user',   content: buildAssistPrompt(body as Parameters<typeof buildAssistPrompt>[0]) },
      ],
    });
